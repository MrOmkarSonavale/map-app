...
class App {
    ...
    _getPosition() {
        if (navigator.geolocation) navigator.geolocation.getCurrentPosition(this._loadMap.bind(this), function(err) {
            console.error(err);
        });
    }
    _loadMap(position) {
    ...
    _newWorkout(ev) {
        ...
        if (type === "running") {
            ...
            if (!validInput(distance, duration, cadence) || !numberPositive(distance, duration, cadence)) return alert("Input have to be positive");
            workout = new Running([
                lat,
                lng
        ...
        if (type === "cycling") {
            ...
            if (!validInput(distance, duration, elevation) || !numberPositive(distance, duration)) return alert("Input have to be positive");
            workout = new Cycling([
                lat,
                lng
    ...
    _renderWorkoutMarker(workout) {
        ...
        })).setPopupContent(`${workout.type === "cycling" ? "\uD83D\uDEB4‍♀️" : "\uD83C\uDFC3‍♂️"} ${workout.description}`).openPopup();
    }
